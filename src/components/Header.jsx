    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'bg-white shadow-lg py-3' : 'bg-white/95 backdrop-blur-sm py-4'}`}>
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between">
          ...
          <a href="#programs" onClick={(e) => scrollToSection(e, 'programs')} className="hidden xl:block bg-emerald-500 text-white px-6 py-2 rounded-full hover:bg-emerald-600 hover:shadow-lg transition-all duration-300 text-sm transform hover:scale-105">Enroll</a>
        ...
        <div className={`xl:hidden overflow-hidden transition-all duration-500 ${isMobileMenuOpen ? 'max-h-96 opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
          <div className="bg-white border-t py-4 space-y-2">
            {navItems.map((item, i) => (
              <a key={i} href={`#${item.toLowerCase()}`} onClick={(e) => scrollToSection(e, item)} className="block py-2 text-gray-800 hover:text-emerald-500 hover:translate-x-2 transition-all duration-300">{item}</a>
            ))}
          </div>
        </div>
      </div>
